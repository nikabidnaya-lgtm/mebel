...
CATEGORY_RULES: Dict[str, Dict[str, Any]] = {
    "chair": {
        ...
        "keywords": [
            "кресл", "стул", "chair", "кресло", "стуль", "табурет", "пуф",
        ],
        "sites": {
            "profoffice": ["/catalog/chairs/"],
            "riva": ["/catalog/kresla/"],
            "attento": ["/kresla/", "/stulya/"],
        },
    },
    "soft": {
        "label": "Мягкая мебель",
    ...
    "office": {
        ...
        "keywords": ["офис", "кабинет", "мебел", "system", "система"],
        "sites": {
            "profoffice": ["/catalog/offices/"],
            "riva": ["/catalog/mebel-dlya-personala/"],
            "attento": ["/operativnaya-mebel/", "/kabinety-rukovoditeley/"],
...
SITE_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "riva": {
        ...
        "price_selectors": ["[itemprop='price']", ".price", ".catalog-element__price", "[class*='price']"],
...
STOP_WORDS = {
    "тип", "для", "и", "с", "со", "на", "по", "из", "под", "над", "без", "мебель", "комплект",
}
...
def build_scrape_plan(df: pd.DataFrame) -> Dict[str, List[str]]:
...
def save_category_cache(site_key: str, cat_path: str, products: List[Dict[str, Any]]) -> None:
    cache_file = category_cache_path(site_key, cat_path)
    cache_file.write_text(json.dumps(products, ensure_ascii=False, indent=2), encoding="utf-8")
...
async def scrape_product_page(page: Page, site_key: str, url: str) -> Optional[Dict[str, Any]]:
    ...
    price_text = await first_text(page, config["price_selectors"], timeout=4000)
    if clean_price(price_text) == 0:
        body_text = await first_text(page, ["body"], timeout=3000)
        match = re.findall(r"(\d{1,3}(?:[\s\u00A0]?\d{3})+|\d+)\s*(?:₽|руб\.?|RUB)", body_text, flags=re.I)
        if match:
            price_text = match[0]
...
async def scrape_categories_by_plan(context: BrowserContext, plan: Dict[str, List[str]], sem_product: asyncio.Semaphore) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for result in results:
        ...
        category_key = next(
            key for key, rule in CATEGORY_RULES.items() if cat_path in rule["sites"].get(site_key, [])
        )
        category_buckets[category_key].extend(products)

    for category_key in list(category_buckets.keys()):
        category_buckets[category_key] = list({item["url"]: item for item in category_buckets[category_key]}.values())
...
def rank_analogs(item: Dict[str, Any], analogs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    query = item["query"]
    ...
    return sorted(candidates, key=lambda row: row["final_score"], reverse=True)[:MAX_ANALOGS_PER_ITEM]
...
def log_excel_diagnostics(excel_path: str, raw_df: pd.DataFrame) -> None:
    ...
    logger.info(f"🧪 Диагностика Excel: absolute={path.resolve() if path.is_absolute() else (Path.cwd() / path).resolve()}")
    logger.info(f"🧪 Диагностика Excel: exists={path.exists()} | suffix={path.suffix} | size_mb={(path.stat().st_size / (1024 * 1024)):.2f if path.exists() else 0:.2f}")
    ...
    preview_names = raw_df.head(8).to_dict(orient="records")
    logger.info(f"🧪 Диагностика Excel: первые строки={preview_names}")
...
def prepare_items(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["query"] = df.apply(
        lambda row: " ".join(
            part for part in [
                str(row.get("Наименование", "")).strip(),
                str(row.get("Описание", "")).strip(),
                str(row.get("Размер ш/г/в", "")).strip(),
            ] if part
        ),
        axis=1,
    )
...
async def main():
    ...
    scrape_plan = build_scrape_plan(df)
    ...
    async with async_playwright() as pw:
        ...
        analog_pools = await scrape_categories_by_plan(context, scrape_plan, sem_product)
        await context.close()
    ...
    for category_key, items in analog_pools.items():
        for item in items:
            dump_row = dict(item)
            dump_row["predicted_category"] = category_key
            all_dump_rows.append(dump_row)
    if all_dump_rows:
